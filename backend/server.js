...
const fs = require("fs")
//importar utilidades para trabalhar dom caminhos de arquivos
...
//tudo da pasta data/fotos pode ser usado pela url/fotos

app.use(
    "/fotos", //rota publica
    express.static(
        path.join(__dirname,"data/fotos")
        //caminho real da parta do servidor
    )
)
...
function sortear(array){
    ...
    //Math.random() gerar um numero entre 0 e 1
    //array.legth pega o tamanho od array (5 itens)
    //Math.floor arredondar o numero para baixo

    //retornar o item sorteado
    return array[i];
}
...
//==========
//---------------------------------
//===1 - rota / cão aleatório
//---------------------------------
//http://localholst:3000/api/cachorros/aleatório

app.get("/api/cachorro/aleatorio",(req,res)=>{
    //pega todas as fotos de todas as raças 
    //object.values oega os valores do objeto
    //flat transformartudo em um unico array
    const todasAaFotos = Object.values(cachorros).flat();

    //sortear uma foto aleatória
    const item =sortear(todasAaFotos)
    //responder ao cliene em formato json
    res.json({
        status: "sucess",
